...
import json
from simulator.src.utils import convert_stat_to_bonus
from simulator.src.actions import PhysicalSingleAttack, SpellSingleAttack, \
    ComboAttack
...
def create_multiattack(creature_name, attack_description, other_actions):
    ...
    Args:
        ...
        other_actions: a list of other actions this creature can take

    Returns:
        a multi-attack action that uses the actions defined in the description
    """
...
if not path.exists(BASE_DIR + "/data/source_monsters.json"):
    print("Please go to "
          "https://dl.dropboxusercontent.com/s/iwz112i0bxp2n4a/"
          "5e-SRD-Monsters.json and download the json file there and place "
          "it in the data directory to continue.")

with open(BASE_DIR + "/data/source_monsters.json", 'r') as f:
    source = json.load(f)
...
for entry in source:
    ...
    am = ActionManager()
    combatant_actions = []
    consumed_actions = []  # a list of json objects that turned into actions
    ...
    # If there are no actions, we don't know how to process any of the
    # combatant's actions yet so skip that combatant for now
